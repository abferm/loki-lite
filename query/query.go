// Package query parses LogQL strings and provides pipeline types that process
// journal entries through Loki-compatible log and metric pipelines.
package query
...
// Entry converts a journal.Entry into an loghttp-compatible representation.
...
// Entry is a simplified log entry representation that carries the processed
// line, stream labels, and structured metadata.
type Entry struct {
	Timestamp          time.Time
	Line               string
...
// Builder constructs LogQL pipelines from a schema.
type Builder struct {
	schema Schema
}

// NewBuilder creates a Builder that uses the given schema for entry conversion.
